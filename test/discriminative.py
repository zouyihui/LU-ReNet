...
def calculate_means(pred, gt, n_objects, max_n_objects, usegpu):
    ...
    means = []
    for i in range(bs):
        _n_objects_sample = n_objects[i] #每个图通道数
...
def discriminative_loss(input, target, n_objects,max_n_objects, delta_v, delta_d, norm, usegpu):
    ...
    for i in range(len(input)):
        bs, n_filters, height, width = input[i].size()
        n_instances = target[i].size(1)

        input[i] = input[i].permute(0, 2, 3, 1).contiguous().view(bs, height * width, n_filters)
        target = target[i].permute(0, 2, 3, 1).contiguous().view(bs, height * width, n_instances)

        cluster_means = calculate_means(input[i], target[i], n_objects, max_n_objects, usegpu)

        var_term = calculate_variance_term(input[i], target[i], cluster_means, n_objects, delta_v, norm)
...
class DiscriminativeLoss(_Loss):
    ...
    def __init__(self, delta_var, delta_dist, norm,size_average=False, reduce=False, usegpu=True):
